...
def build_isolated_sign_model(
    ...
    """Xây dựng mô hình nhận diện ký hiệu tay dựa trên Transformer.

    - Khi use_ctc_output=True: trả về logits theo từng khung (batch, T, C+1) để dùng CTC.
    - Khi use_ctc_output=False: phân loại trực tiếp bằng masked average pooling và softmax.
    """
    ...
    # Chiếu đặc trưng + positional embedding học được
    ...
    # Attention mask (batch, T, T)
    attention_mask = tf.keras.layers.Lambda(
        lambda m: tf.cast(
            tf.repeat(tf.expand_dims(m, axis=1), repeats=max_frames, axis=1),
            tf.float32,
        ),
        name="attention_mask",
    )(frame_mask)
    ...
    for block_idx in range(num_transformer_blocks):
        ...
        attn_output = tf.keras.layers.Dropout(
            dropout_rate, name=f"attn_dropout_{block_idx}"
        )(attn_output)
        x = tf.keras.layers.LayerNormalization(
            epsilon=1e-6, name=f"attn_layer_norm_{block_idx}"
        )(x + attn_output)
        x = tf.keras.layers.Multiply(name=f"mask_after_attn_{block_idx}")(
            [x, frame_mask_float]
        )

        ffn_output = tf.keras.layers.Dense(
            ffn_dim, activation=tf.nn.gelu, name=f"ffn_expand_{block_idx}"
        )(x)
        ffn_output = tf.keras.layers.Dropout(
            dropout_rate, name=f"ffn_dropout_{block_idx}_1"
        )(ffn_output)
        ffn_output = tf.keras.layers.Dense(
            feature_dim, name=f"ffn_project_{block_idx}"
        )(ffn_output)
        ffn_output = tf.keras.layers.Dropout(
            dropout_rate, name=f"ffn_dropout_{block_idx}_2"
        )(ffn_output)
        x = tf.keras.layers.LayerNormalization(
            epsilon=1e-6, name=f"ffn_layer_norm_{block_idx}"
        )(x + ffn_output)
        x = tf.keras.layers.Multiply(name=f"mask_after_ffn_{block_idx}")(
            [x, frame_mask_float]
        )
    ...
    if use_ctc_output:
        # CTC loss sẽ tự áp dụng softmax bên trong
        logits = tf.keras.layers.Dense(
            num_classes + 1, activation=None, name="ctc_logits"
        )(x)
        model = tf.keras.Model(
            inputs=[inputs, frame_lengths], outputs=logits, name="IsolatedSignModelCTC"
        )
    else:
        # Masked average pooling theo độ dài thật
        summed = tf.keras.layers.Lambda(
            lambda tensor: tf.reduce_sum(tensor, axis=1), name="masked_sum"
        )(x)
        valid_lengths = tf.keras.layers.Lambda(
            lambda seq_len: tf.maximum(
                tf.cast(tf.expand_dims(seq_len, axis=-1), tf.float32), 1.0
            ),
            name="safe_lengths",
        )(frame_lengths)
        pooled = tf.keras.layers.Lambda(
            lambda args: args[0] / args[1], name="masked_average"
        )([summed, valid_lengths])
        pooled = tf.keras.layers.Dense(
            256, activation="relu", name="classifier_dense"
        )(pooled)
        ...
        model = tf.keras.Model(
            inputs=[inputs, frame_lengths], outputs=outputs, name="IsolatedSignModel"
        )
        model.compile(
